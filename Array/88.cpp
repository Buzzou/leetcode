...
public:
    static vector<int> merge(vector<int>& nums1, int m, vector<int>& nums2, int n)
    {
        ...
        {
            std::merge(nums1.begin(), nums1.begin() + m, nums2.begin(), nums2.end(), temp.begin());
            // copy(nums1.begin(), nums1.begin()+m, ostream_iterator<int>(cout, " "));
            // cout << endl;
            // copy(nums2.begin(), nums2.end(), ostream_iterator<int>(cout, " "));
            // cout << endl;

            // assert(temp.size() == nums1.size());
            nums1 = std::move(temp);
        }
